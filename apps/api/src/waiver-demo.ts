...
export function demoWaiverInput(now = new Date()): WaiverInput {
  ...
  const signals: PlayerSignal[] = entries.map(([playerId, , position, weekly, future]) => ({ playerId,
    weeks: Array.from({ length: 10 }, (_, i) => ({ week: i + 8, stats: { rush_yd: position === 'RB' ? weekly * 10 : 0, rec: position === 'RB' ? 0 : weekly / 2, rec_yd: position === 'RB' ? 0 : weekly * 5 }, opponent: ['SAMPLE A', 'SAMPLE B', 'SAMPLE C'][i % 3], bye: playerId === 'starter-te' && i === 0, matchupMultiplier: i > 7 ? 1.1 : 1 })),
