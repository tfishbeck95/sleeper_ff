...
test('dashboard metadata and recommendation APIs share persisted scoring and fail closed on refresh errors', async () => {
  ...
  process.env.ENABLE_DEMO_AUTH='true'; const login=await request(app).post('/auth/demo'); const get = (path: string) => request(app).get(path).set('Cookie',login.headers['set-cookie'][0].split(';')[0]);
