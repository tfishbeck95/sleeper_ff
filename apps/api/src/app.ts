...
export function createApp(store: JsonStore, sleeper = new SleeperClient(), sync = new LeagueSyncService(store, sleeper), signals: WaiverSignalProvider = new FileWaiverSignalProvider()) { const app=express(); app.use(cors({origin:process.env.WEB_ORIGIN ?? 'http://localhost:5173'})); app.use(express.json());
 ...
 app.get('/api/dashboard/:leagueId',async(req,res)=>{let data=await store.snapshot(req.params.leagueId); if(!data&&req.params.leagueId==='demo'){data=demoSnapshot();await store.save(data);} if(!data)return res.status(404).json({error:'League not synced'});res.json(req.params.leagueId === 'demo' ? data : { ...data, scoring: (await store.league(req.params.leagueId))?.scoring ?? scoringUnavailable(), recommendations: [] });});
