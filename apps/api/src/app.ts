...
function createRateLimiter(){const limits=new Map<string,{start:number,count:number}>();return (bucket:string,max:number,windowMs:number)=>(req:express.Request,res:express.Response,next:express.NextFunction)=>{const key=`${bucket}:${req.ip}`;const now=Date.now();let value=limits.get(key);if(!value||now-value.start>=windowMs)value={start:now,count:0};value.count++;limits.set(key,value);if(limits.size>10_000)for(const [candidate,record]of limits)if(now-record.start>=windowMs)limits.delete(candidate);res.set('RateLimit-Limit',String(max));res.set('RateLimit-Remaining',String(Math.max(0,max-value.count)));if(value.count>max)return res.status(429).set('Retry-After',String(Math.ceil((value.start+windowMs-now)/1000))).json({error:'Too many requests. Try again later.'});next();};}
...
export function createApp(store: JsonStore, sleeper = new SleeperClient(), sync = new LeagueSyncService(store, sleeper), signals: WaiverSignalProvider = new FileWaiverSignalProvider()) { const app=express();const rateLimit=createRateLimiter();app.disable('x-powered-by'); app.use(cors({origin:process.env.WEB_ORIGIN ?? 'http://localhost:5173', credentials:true})); app.use(express.json({limit:'32kb'}));
 ...
 app.post('/auth/demo', rateLimit('demo-login', 10, 15*60_000), async(_req,res)=>{if(process.env.NODE_ENV==='production'||process.env.ENABLE_DEMO_AUTH!=='true')return res.status(404).json({error:'Not found.'});let user=await store.applicationUserByLogin('demo');if(!user){user={id:randomUUID(),login:'demo',passwordHash:'disabled',sleeperUserId:'sample',sleeperUsername:'sample',sleeperLeagueIds:(process.env.DEMO_SLEEPER_LEAGUE_IDS ?? 'demo').split(',').map(id=>id.trim()).filter(Boolean),createdAt:new Date().toISOString()};await store.saveApplicationUser(user);}const issued=await issueSession(store,user);res.set('Set-Cookie',sessionCookie(issued.rawSessionId,issued.maxAge)).json({user:publicUser(user),csrfToken:issued.csrfToken,expiresAt:issued.session.expiresAt});});
 const requireAuth=authentication(store);
 app.get('/auth/session',requireAuth,(_req,res)=>{const auth=res.locals.auth as Authentication;res.set('Cache-Control','no-store').json({user:publicUser(auth.user),csrfToken:auth.session.csrfToken,expiresAt:auth.session.expiresAt});});
 ...
 app.use('/api',(req,res,next)=>Object.hasOwn(req.query,'userId')?res.status(400).json({error:'userId is derived from the authenticated session and must not be supplied.'}):next());
 app.use('/api',csrf);
 ...
 app.get('/api/sleeper/leagues',rateLimit('username',20,60_000),async(req,res,next)=>{try{const linked=(res.locals.auth as Authentication).user.sleeperUserId;if(!linked)return res.status(409).json({error:'Link a Sleeper account first.'});const current=new Date().getUTCFullYear();const requested=typeof req.query.seasons==='string'?req.query.seasons.split(','):[current,current-1,current-2].map(String);const seasons=requested.filter(season=>/^\d{4}$/.test(season)).slice(0,6);if(!seasons.length)return res.status(400).json({error:'Provide at least one valid season.'});const results=await Promise.all(seasons.map(async season=>({season,leagues:await sleeper.leagues(linked,season)})));res.json({seasons:results,lastSyncedAt:new Date().toISOString()});}catch(error){next(error);}});
