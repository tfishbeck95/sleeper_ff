...
test('dashboard renders the requested section order with accessible status and navigation', () => {
  const html = renderToStaticMarkup(<Dashboard connection={null} onConnect={noop} onDemo={noop}/>);
...
test('a connected dashboard first renders loading, without leaking demo recommendations', () => {
  const input = fixture();
  const html = renderToStaticMarkup(<Dashboard connection={{ user: input.users[0], leagues: [{ league: input.league, roster: input.rosters[0], coOwned: false }] }} onConnect={noop} onDemo={noop}/>);
