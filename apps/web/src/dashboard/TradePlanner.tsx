...
export function TradePlanner({ leagueId, userId, week, demo, force = false, shared }: { leagueId: string; userId?: string; week: number; demo: boolean; force?: boolean; shared?: { report: TradeReport | null; error: string; loading: boolean; recheck: () => void; bounds: { maxValueGap: string; maxRisk: string }; onBounds: (bounds: { maxValueGap: string; maxRisk: string }) => void } }) {
  ...
  const [retry, setRetry] = useState(0), [gap, setGap] = useState('0.25'), [risk, setRisk] = useState('0.65'), [format, setFormat] = useState('redraft');
  ...
  return <div className="surface trade-planner" aria-busy={busy}><div className="trade-planner-heading"><strong>Mutual roster fit</strong><button className="secondary-button" disabled={busy} onClick={() => shared ? shared.recheck() : setRetry(v => v + 1)}>Recheck trades</button></div>
    <div className="trade-filters"><label>Maximum value gap<select value={shared?.bounds.maxValueGap ?? gap} onChange={e => shared ? shared.onBounds({ ...shared.bounds, maxValueGap: e.target.value }) : setGap(e.target.value)}><option value="0.1">10% · Strict</option><option value="0.25">25% · Balanced</option><option value="0.4">40% · Flexible</option></select></label><label>Maximum risk index<select value={shared?.bounds.maxRisk ?? risk} onChange={e => shared ? shared.onBounds({ ...shared.bounds, maxRisk: e.target.value }) : setRisk(e.target.value)}><option value="0.3">30 / 100</option><option value="0.65">65 / 100</option><option value="0.9">90 / 100</option></select></label>{demo && <label>Sample format<select value={format} onChange={e => setFormat(e.target.value)}><option value="redraft">Redraft</option><option value="dynasty">Dynasty</option></select></label>}</div>
