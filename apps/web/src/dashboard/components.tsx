...
export function MatchupCard({ data }: { data: DashboardData }) {
  ...
  const margin = matchup.projectedFor != null && matchup.projectedAgainst != null ? matchup.projectedFor - matchup.projectedAgainst : null;
  return <div className="matchup-card"><div className="matchup-kicker"><span><span className="live-dot"/> WEEK {data.week} OUTLOOK</span><span>{data.demo ? 'Sample projections' : 'League matchup'}</span></div><div className="matchup-teams"><div><span className="team-mark home" aria-hidden="true"><Flag size={22}/></span><strong>{data.teamName}</strong><span>You</span></div><span className="versus">VS</span><div><span className="team-mark away" aria-hidden="true"><Zap size={22}/></span><strong>{matchup.opponent}</strong><span>Opponent</span></div></div>
    <div className="score-comparison"><div><strong>{number(matchup.projectedFor)}</strong><span>projected points</span></div><span className="score-dash">:</span><div><strong>{number(matchup.projectedAgainst)}</strong><span>projected points</span></div></div>
    {matchup.actualFor != null && <p className="actual-score">Current score: {number(matchup.actualFor)} – {number(matchup.actualAgainst)}</p>}
    <div className="matchup-odds"><div><span>{margin == null ? 'Projection unavailable' : margin === 0 ? 'Evenly projected' : margin > 0 ? 'A slight edge for you' : 'You’re the projected underdog'}</span><strong>{matchup.winChance == null ? 'Odds unavailable' : `${matchup.winChance}% to win`}</strong></div>{matchup.winChance != null && <div className="win-meter" role="img" aria-label={`${matchup.winChance}% sample chance to win, ${100 - matchup.winChance}% chance to lose`}><span style={{ width: `${matchup.winChance}%` }}/></div>}<p>{margin == null ? 'A projection source is needed to estimate your edge.' : `${signed(margin)} projected point margin · ${data.demo ? 'Illustrative estimate' : 'Not a guaranteed outcome'}`}</p></div>
