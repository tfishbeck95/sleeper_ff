...
import { fromLeagueDetails, sleeperLeagueUrl, sortAlerts, syncAlert } from './model';
...
export function Dashboard({ connection, onConnect, onDemo, onLogout }: { connection: DashboardConnection | null; onConnect: () => void; onDemo: () => void; onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    loadLeague(leagueId, week, controller.signal).then(({ details, snapshot }) => {
      if (controller.signal.aborted) return;
      setResult({ key, data: fromLeagueDetails(details, connection.user.user_id, week, snapshot) });
  ...
  }, [connection, leagueId, week, reload, key]);
  ...
  const refresh = () => { if (connection) setReload(v => v + 1); else setStatus('Sample scenario reloaded. This is illustrative data, not a live sync.'); };
  ...
  // One league-scored lineup report serves both the start/sit and matchup sections.
  const lineup = useLineupReport({ leagueId, userId: connection?.user.user_id, week: data?.week ?? week, demo: data?.demo ?? !connection, force: reload > 0 });
  return <div className="dashboard-shell"><a className="skip-link" href="#dashboard-main">Skip to dashboard</a>
    ...
    <div className="dashboard-workspace"><header className="topbar"><div className="breadcrumb"><LayoutDashboard size={17}/><span>Workspace</span><ChevronRight size={14}/><strong>{navigation.find(item => item.id === activeNav)?.label}</strong></div><a className="mobile-brand" href="#overview">huddle.</a><div className="topbar-actions"><span className="read-only"><ShieldCheck size={15}/> Read-only</span><a className="icon-button notification-button" href="#alerts" aria-label={`${alerts.length} alerts. Go to urgent alerts.`}><Bell size={20}/>{alerts.length > 0 && <span/>}</a><button className="topbar-avatar" aria-label="Log out" title="Log out" onClick={onLogout}>{initials(connection?.user.display_name ?? 'Sample Manager')}</button><button className="icon-button mobile-menu-button" aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={menuOpen} aria-controls="mobile-navigation" onClick={() => setMenuOpen(v => !v)}>{menuOpen ? <X/> : <Menu/>}</button></div></header>
    ...
    {busy && !data ? <LoadingDashboard/> : !data ? <div className="surface initial-error" role="alert"><EmptyState icon={<CircleAlert/>} title="We couldn’t load your league">{error || 'Your league data is not available yet.'} No recommendations are being shown.</EmptyState><div><Button onClick={refresh}><RefreshCw size={16}/> Try again</Button><button className="secondary-button" onClick={onDemo}>Explore sample dashboard</button></div></div> : <>
    <ScoringStatus scoring={error ? scoringUnavailable('Live scoring refresh failed. Retry before using rankings.') : data.scoring}/>
    ...
    <div className="market-layout"><section id="waivers" aria-labelledby="waivers-title"><SectionHeader id="waivers-title" number="03" title="Find your next upgrade" subtitle="Waiver recommendations, ranked by roster fit and value."/><WaiverPlanner key={`${key}:${reload}`} leagueId={leagueId} userId={connection?.user.user_id} week={data.week} demo={data.demo} force={reload > 0}/></section>
    <section id="trades" aria-labelledby="trades-title"><SectionHeader id="trades-title" number="04" title="Build a stronger roster" subtitle="Your needs. A potential fit across the league."/><TradePlanner key={`${key}:${reload}`} leagueId={leagueId} userId={connection?.user.user_id} week={data.week} demo={data.demo} force={reload > 0}/></section></div>
    ...
    </main></div>{action && <ActionDialog key={`${key}:${action.id}`} action={action} leagueId={leagueId} demo={!connection} onClose={() => setAction(null)} onRetry={refresh} onConnect={onConnect}/>}</div>;
}
