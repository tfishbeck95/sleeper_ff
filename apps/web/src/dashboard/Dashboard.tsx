...
export function Dashboard({ connection, onConnect, onDemo }: { connection: DashboardConnection | null; onConnect: () => void; onDemo: () => void }) {
  ...
  return <div className="dashboard-shell"><a className="skip-link" href="#dashboard-main">Skip to dashboard</a>
    ...
    <div className="decision-layout"><section id="lineup" aria-labelledby="lineup-title"><SectionHeader id="lineup-title" number="01" title="Make the right starts" subtitle="Small lineup changes. More points on the board."/>{data.starts.length ? <div className="start-grid">{data.starts.map(decision => <StartCard key={decision.id} decision={decision} onReview={setAction}/>)}</div> : <div className="surface"><EmptyState icon={<ListChecks/>} title="Start/sit analysis isn’t ready">Player projections and slot comparisons are needed before an advantage or confidence can be shown. Review your lineup alerts above.</EmptyState></div>}<p className="section-footnote"><Info size={13}/> Projected advantages are estimates. Review each checklist; changes are confirmed in Sleeper.</p></section>
    <section id="matchup" aria-labelledby="matchup-title"><SectionHeader id="matchup-title" number="02" title="Your matchup" subtitle="Know your edge. Plan for the swing."/><MatchupCard data={data}/></section></div>
